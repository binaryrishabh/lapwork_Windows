...
function TimeChart({ sessions, chartType, setChartType, stats }) {
  ...
  const { bars, displayTotalMs } = useMemo(() => {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        normalizedSessions.forEach((s) => {
          if (s.dateStr === dateStr) {
            ...
            dayDistractedMs += s.distractions.reduce(
        ...
        const dayFocusRate =
          dayTotalMs > 0 ? Math.round((dayProductiveMs / dayTotalMs) * 100) : 0;
        const dayAvgSession =
          daySessions > 0 ? dayProductiveMs / daySessions : 0;
    ...
    if (chartType === "yearly") {
      ...
      for (let month = 0; month < 12; month++) {
        ...
        normalizedSessions.forEach((s) => {
          ...
          if (parts.length === 3) {
            if (
              ...
              monthDistractedMs += s.distractions.reduce(
                (sum, d) => sum + d.durationMs,
                0,
              );
              monthSessions++;
        ...
        const monthFocusRate =
          monthTotalMs > 0
            ? Math.round((monthProductiveMs / monthTotalMs) * 100)
            : 0;
        const monthAvgSession =
          monthSessions > 0 ? monthProductiveMs / monthSessions : 0;
  ...
  if (chartType === "monthly") {
    ...
    barWidth = Math.max(
      6,
      Math.floor((plotWidth - totalGaps * barGap) / totalBars),
    );
  ...
  const totalWidth =
    totalBars * barWidth + totalGaps * barGap + paddingX + containerPadding;

  // Y-axis
  const maxHours = Math.max(
    ...bars.filter((b) => !b.isFuture).map((b) => b.hours),
    0.1,
  );
  const yLabels = [];
  if (maxHours <= 0.5) {
    ...
    <div className="chart-container">
      ...
      <div className="chart-scroll" ref={chartScrollRef}>
        ...
        >
          {yLabels.map((val, i) => {
            const ratio =
              maxHours <= 0.5 ? val / 60 / maxHours : val / maxHours;
          ...
          {bars.map((bar, i) => {
            ...
            const showValue =
              bar.hours > 0 && !(chartType === "monthly" && barWidth < 12);
              ...
              <g key={i}>
                {bar.isFuture ? (
                  ...
                  <rect
                    ...
                    width={barWidth}
                    height={barH}
                    rx={chartType === "monthly" ? 1 : 4}
                    className={`chart-bar ${isHighlighted ? "highlighted" : ""}`}
                    onMouseEnter={(e) =>
                      bar.tooltipData &&
                      bar.tooltipData.totalMs > 0 &&
                      handleBarEnter(e, bar)
                    }
                    onMouseMove={(e) => tooltip && handleBarMove(e)}
                    onMouseLeave={handleBarLeave}
                    style={{
                      cursor:
                        bar.tooltipData && bar.tooltipData.totalMs > 0
                          ? "pointer"
                          : "default",
                    }}
                  />
                ...
                {xLabel !== "" && (
                  <text
                    ...
                    className={`chart-bar-label ${isHighlighted ? "highlighted" : ""}`}
                    textAnchor="middle"
                  >
                    {xLabel}
                  </text>
                ...
                {showValue && (
                  <text
                    x={x + barWidth / 2}
                    y={y - 6}
                    className={`chart-bar-value ${isHighlighted ? "highlighted" : ""}`}
                    textAnchor="middle"
                    fontSize={chartType === "monthly" ? "8" : "10"}
                  >
                    {formatMs(bar.ms)}
                  </text>
                )}
          ...
          >
            ...
            <div className="tooltip-stats">
              <div className="tooltip-stat-row">
                ...
                <span className="tooltip-stat-value">
                  {formatMs(tooltip.data.totalMs)}
                </span>
              </div>
              <div className="tooltip-stat-row">
                <span className="tooltip-stat-label">Avg Session</span>
                <span className="tooltip-stat-value">
                  {formatMs(tooltip.data.avgSessionMs)}
                </span>
              </div>
              <div className="tooltip-stat-row">
                ...
                <span
                  ...
                  style={{
                    color:
                      tooltip.data.focusRate >= 80
                        ? "var(--success)"
                        : tooltip.data.focusRate >= 60
                          ? "var(--warning)"
                          : "var(--error)",
                  }}
                >
            ...
            {tooltip.data.sessions > 0 && (
              <div className="tooltip-sessions">
                {tooltip.data.sessions} session
                {tooltip.data.sessions !== 1 ? "s" : ""}
              </div>
            )}
          </div>
        )}
